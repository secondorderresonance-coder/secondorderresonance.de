...
- Added `gradeRange` field to all 15 direct L1-L3 taxonomy sublevels in `app/data/archive-content.js` (1.1–1.4, 2.1–2.6, 3.1–3.5), covering Klasse 1-6 (L1), Klasse 7-10 (L2), and Klasse 11-12 (L3).
- Updated `app/lernpfad.html` section header to display the grade range alongside the task count (e.g. "1.234 Archivaufgaben · Klasse 7-9").
- Created `docs/curriculum-coverage-matrix.md` with the full Klasse 1-12 → L1-L3 mapping table, gap analysis, and coverage summary.
...
- Grep confirms 15 `gradeRange` entries in the taxonomy, one per direct sublevel (1.1–3.5)
- Lernpfad section-meta template updated with conditional grade label
- `git diff --check`
